...
import { Terminal, Info, XCircle, ArrowLeft, CheckCircle } from "lucide-react"
import { cn } from "@/lib/utils"
import {
  cultureData as initialCultureData,
  type CultureData,
  type CulturalWarning,
  type TouristSpot,
} from "@/lib/culture-data"
import { allCountries, TOTAL_UN_MEMBERS } from "@/lib/countries"
...
export default function Component() {
  ...
  // Cache for all loaded cultural data
  const [allCultureData, setAllCultureData] = useState<Map<string, CultureData>>(() => {
    const initialMap = new Map<string, CultureData>()
    initialCultureData.forEach((data) => initialMap.set(data.id, data))
    return initialMap
  })

  // Set of IDs of countries for which data has been loaded
  const [loadedCountryIds, setLoadedCountryIds] = useState<Set<string>>(() => {
    const initialSet = new Set<string>()
    initialCultureData.forEach((data) => initialSet.add(data.id))
    return initialSet
  })

  const filteredDestinations = useMemo(() => {
    if (!destinationInput) return []
    const lowerCaseInput = destinationInput.toLowerCase()

    // Filter from all known countries, not just loaded ones
    return allCountries.filter((c) => c.name.toLowerCase().includes(lowerCaseInput))
  }, [destinationInput])

  const handleSelectDestination = async (selectedId: string) => {
    setLoading(true)
    setError(null)

    // Check if data is already in cache
    if (allCultureData.has(selectedId)) {
      const foundCulture = allCultureData.get(selectedId)!
    ...
    try {
      const response = await fetch(`/api/culture/${selectedId}`)
      if (!response.ok) {
        throw new Error("Failed to fetch cultural data.")
      }
      const data: CultureData = await response.json()

      setAllCultureData((prev) => new Map(prev).set(data.id, data))
      setLoadedCountryIds((prev) => new Set(prev).add(data.id))
  ...
  // Calculate missing countries for display
  const missingCountryCodes = useMemo(() => {
    const loadedIds = new Set(loadedCountryIds)
    return allCountries.filter((country) => !loadedIds.has(country.id)).map((country) => country.id.toUpperCase()) // Use uppercase for ISO codes
  }, [loadedCountryIds])
            ...
            <Card className="w-full max-w-md p-6">
              ...
              <CardContent className="grid gap-4">
                ...
                <div className="text-center text-sm text-muted-foreground mt-4 flex items-center justify-center gap-2">
                  ...
                  <span>
                    {loadedCountryIds.size}/{TOTAL_UN_MEMBERS}カ国ロード済み
                  </span>
                </div>
                {missingCountryCodes.length > 0 && (
                  <div className="text-center text-xs text-muted-foreground mt-2">
                    <p>Missing: {missingCountryCodes.join(", ")}</p>
                  </div>
                )}
              </CardContent>
            </Card>
